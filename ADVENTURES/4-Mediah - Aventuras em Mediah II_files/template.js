jQuery(document).ready(function($){
	...
	$('.js-tag-search-form').submit(function(){
		...
		var vaf=$(this).find('.js-a-search').val();
		if (vaf)  {
			window.location = aRouter['search']+'topics/'+'?q='+encodeURIComponent(vaf);
		}
		if (val) {
			window.location = aRouter['tag']+encodeURIComponent(val)+'/';
		}
		if (vaf && val) {
			window.location = aRouter['search']+'topics/'+encodeURIComponent(val)+'/'+'?q='+encodeURIComponent(vaf);
		}
	...
	ls.autocomplete.add($(".autocomplete-blogs"), aRouter['ajax']+'autocompleter/blogs/', false);
